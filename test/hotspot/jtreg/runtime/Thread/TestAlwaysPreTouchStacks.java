 ...
 * are resident, which is mincore(2) on POSIX.  AIX is excluded because its
 * /proc does not answer, and OpenBSD because it removed mincore altogether,
 * so os::first_resident_in_range() there reports the whole range resident
 * and the two runs report the same number whatever pre-touch did.
 * @library /test/lib
